...
| `contracts/shared` | ErrorResponse（error-standards.md の6コード準拠）・Paging | エラーコード体系（AUX-MST-0001形式）は未確定のため**ダミー採番**と明記して使用 |
...
| P-4 | エラーコード採番（AUX-MST-0001形式）はダミーと明記して使用 | error-standards.md で基本設計確定待ちのため |
...
- スタブ認可 → AUX様提供の認可情報スキーマ確定後に差し替え（判定の置き場は不変）
