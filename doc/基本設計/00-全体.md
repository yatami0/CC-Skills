...
| V-5 | メタ定義駆動 UI | マスタ個別の画面コードなしで、メタ定義差し替えだけで複数マスタ画面が動く | FE | **実証済**。`@poc/ui-engine` ＋ React で同一ページ `<MasterCrudPage>` が dept/price を render→DOM 描画（Next は未導入・本番化軸）（[10-FE.md §8](10-FE.md)・[v-5/検証メモ.md](../../app/contracts_poc/doc/v-5/検証メモ.md)） |
...
- **FE フェーズ（現在・TS-only PoC）**: V-1(TS) / V-2 / V-3 / **V-5 実証済**。FE-native（V-1,2,3,5）が出揃った。V-5 で React + ui-engine を導入して V-3 が真の画面結合（render→DOM）へ昇格し、テストランナー（Vitest）も本決めとなった → **FE 一旦完了**。Next.js 配線は本番化軸（§9 末尾）。
- **BE フェーズ（次）**:
